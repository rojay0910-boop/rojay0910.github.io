### Hi, I'm rojay0910-boop
### Welcome to my data analytics portfolio.  
This portfolio showcases my projects in SQL, Python, data analysis, and data visualization, 
with a growing focus on manufacturing, quality control, and reliability data analysis.

---
### About Me（自己紹介）
- Aspiring **Data Analyst / Data Scientist**   
- Currently learning **Python, Pandas, SQL, data visualization**  
- Building projects using **Jupyter Notebook & GitHub**
- Focusing on clear analytical thinking, reproducible analysis, and well-structured projects
 
###    
- データアナリスト／データサイエンティストを目指しています。  
- Python・Pandas・SQL・データ可視化を勉強中。  
- Jupyter Notebook と GitHub を使って、分析プロジェクトを作成しています。  
- 将来的には製造業・品質管理・信頼性分野のデータ分析に携わることを目標としています。

---
### Projects（プロジェクト）

### 1. GitHub Repository File-Type Analysis (BigQuery + SQL)
**Analyze the public github_repos dataset in Google BigQuery using SQL to demonstrate practical analytics skills (joins, aggregations, window functions) on large-scale real-world data.**   
Google BigQuery の公開データセット（GitHub Activity data / github_repos）を使用し、
SQL による探索的データ分析と指標の設計・作成を行い、JOIN・集計・ウィンドウ関数などの分析スキルを示す。  
[View Project](https://github.com/rojay0910-boop/github-sql-portfolio)

### 2. Foundry-Level Timeline Analysis of Advanced Semiconductor Process Nodes 
**Analyzing the evolution of advanced semiconductor process nodes through a foundry-level technology timeline, focusing on architectural inflection points rather than nominal node labels.**  
ファウンドリーレベルの技術タイムラインを通じて、名目的なプロセスノード表記ではなく、アーキテクチャ上の転換点に焦点を当てながら、先端半導体プロセスノードの進化を分析する。  
[View Project](https://github.com/rojay0910-boop/foundry-process-inflection)  
  
  
More projects coming soon!  

Planned future projects include:  
- Statistical Process Control (SPC) analysis  
- Probability Plot and distribution analysis  
- Weibull life data analysis  
- Yield analysis for manufacturing data  
- Defect analysis and Pareto charts  

今後は以下のようなプロジェクトを追加予定です：  
- 統計的工程管理（SPC）分析  
- Probability Plot（分布判定）  
- ワイブル分布による寿命データ解析  
- 製造データの歩留まり（Yield）分析 
- 不良データのパレート分析  

---
### Learning Journey（学習中の内容）
I’m currently learning Python, SQL, data analysis, and data visualization through platforms such as Codecademy, and applying these skills to real-world datasets.
In addition, I am studying Quality Control and Reliability Engineering, including：
Statistical Process Control (SPC)  
Acceptance Sampling  
Probability distributions (Normal, Lognormal, Weibull, Exponential)  
Probability plotting  
Basic life data analysis
Reliability metrics such as Failure Rate and MTTF

My goal is to apply statistical analysis and data analysis techniques to manufacturing, quality, and reliability-related data in the future.   

###  
Codecademy などの教材を通じて、Python、SQL、データ分析、データ可視化を学習し、実データを用いた分析プロジェクトに取り組んでいます。
また、品質管理および信頼性工学についても学習しており、統計的工程管理（SPC）、抜取検査、確率分布、Probability Plot、寿命データ解析などを勉強しています。
将来的には、製造業や半導体分野における品質・信頼性・製造データ分析にデータ分析技術を応用することを目標としています。

---
### Portfolio Direction（ポートフォリオの方向性）

This portfolio is evolving toward the following areas:  
Data Analysis  
Statistical Analysis  
Manufacturing Data Analysis  
Quality Control  
Reliability Engineering  
Semiconductor Industry Analysis  

データ分析に加えて、今後は以下の分野に関連する分析プロジェクトを増やしていく予定です：  
データ分析  
統計解析  
製造データ分析  
品質管理  
信頼性工学  
半導体産業分析  

---

### Contact

- GitHub: [@rojay0910-boop](https://github.com/rojay0910-boop)

Thanks for visiting!  
